...
TASKS=["alfworld", "jericho", "pddl", "babyai", "scienceworld"]
...
def main():
    ...
    for line in open(os.path.join(log_dir, 'all_results.txt'), "r"):
        ...
        if "_summary" not in line: 
            log_history[json.loads(line.strip())["task_name"]] = json.loads(line.strip()) # load already done tasks
    ...
    for task_name in task_names:
        ...
        if 'tool' in task_name:
            ...
            task = load_task(task_name, run_config, llm_config, agent_task_config, env_config[task_name], llm=llm) # task init, together with agent init
