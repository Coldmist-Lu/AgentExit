...
class EvalScienceworld(BaseTask):
    ...
    def evaluate_env(self, index, task_name, var, modified_goal):
        ...
        self.agent.task_id = f"scienceworld_{index}" # add task id for history load
       ...
       # print(init_obs)
        ...
        for i in range(self.max_num_steps):
            ...
            if isDone:
                ...
                self.agentboard.log_example(index, True, 1.0, grounding_acc_count / (i + 1), score_change_record, env_details, trajectory, extra=extra_details)
        ...
        self.agentboard.log_example(index, isDone, progress_rate, grounding_acc_count / (i + 1), score_change_record, env_details, trajectory, example_prompt, extra=extra_details)
