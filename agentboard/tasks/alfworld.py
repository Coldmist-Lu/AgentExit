...
class Evalalfworld(BaseTask):
    ...
    def evaluate_env(self,  index, ob='', examples=None):
        ...
        self.agent.task_id = f"alfworld_{index}" # add task id for history load
        ...
        for i in range(0, self.max_num_steps):
            ...
            success, action = self.agent.run(init_prompt_dict=init_prompt_dict) # seal token number in action.

            if isinstance(action, dict):
                _token, action = action_dict_process(action, step_id=i, trajectory=trajectory, logger=logger)
                token_cnt += _token
            ...
            if done:
                ...
                self.agentboard.log_example(index, True, reward, grounding_acc_count / (i + 1), score_change_record, env_details, trajectory, extra=extra_details)
                
                return 1.0, True, grounding_acc_count / (i + 1), score_change_record, i

        # deal with unsuccess scenario
        game_name = self.env.cur_task_name.split('/')[0]
        env_details = {"task_name": game_name, "goal": self.agent.goal, "difficulty": self.env.difficulty}
        ...
        self.agentboard.log_example(index, done, progress_rate, grounding_acc_count / (i + 1), score_change_record, env_details, trajectory, example_prompt, extra=extra_details)
