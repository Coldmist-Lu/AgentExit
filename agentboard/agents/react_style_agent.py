...
# AgentExit: We follow Expolration-Trajectory Optimization
@registry.register_agent("ReactStyleAgent") # Agent with react format
class ReactStyleAgent(BaseAgent):  # the agent should receive goal, state and action, then return the next state
    def __init__(self,
                 llm_model,
                 memory_size=100,
    ...
    def update(self, action='', state=''):
        self.steps += 1
        # self.memory.append(("Action", action)) # has thought and action in run.
    ...
    def from_config(cls, llm_model, config):
        ...
        return cls(llm_model, memory_size, examples, instruction, init_prompt_path, system_message, 
                   need_goal, check_actions, check_inventory, use_parser, intrinsic_exit) 
