...
from .utils import load_multiple_json_objects
...
@registry.register_agent("ReactStyleAgentHistory") # Agent with react format
class ReactStyleAgentHistory(BaseAgent):  # the agent should receive goal, state and action, then return the next state
    def __init__(self,
                 llm_model,
                 memory_size=100,
                 ...
                 use_parser=True,
                 intrinsic_exit="short_thought", # add intrinsic exit
                 history_file=None,
    ...
    def reset(self, goal, init_obs, init_act=None, env=None):
        ...
        if self.history_file is not None:
            ...
            history_file = load_multiple_json_objects(os.path.join(self.history_file, "logs", f"{task}.jsonl"))
            task_index = int(task_index)
            self.history_log = history_file[task_index]
            assert task_index == self.history_log['id'], "Task_id doesn't match!"


        self.reflection_str = ""
        if self.reflection_path is not None:
            with open(self.reflection_path, 'r', encoding='utf-8') as f:
                reflection_file = json.load(f)

            task_id = int(self.task_id.split("_")[-1])
            self.reflection_str = reflection_file[task_id].get("reflection", "")
    ...
    def update(self, action='', state=''):
        self.steps += 1
        # self.memory.append(("Action", action)) # update before next check.
    ...
    def make_prompt(self,
        ...
        if history_message[0]['role'] == "user":
        ...
        # add early_exit
        if self.intrinsic_exit is not None:
            prompt_message[-1]['content'] += INTRINSIC_PROMPT[self.intrinsic_exit]

        if self.reflection_str != "":
            prompt_message[-1]['content'] += "\n **Important** You may proceed with the following new plan for further exploration:" + self.reflection_str + "\n"
    ...
    def run(self, init_prompt_dict=None):
        ...
        if self.steps < self.history_log['extra_record']['steps']:
            # find interaction dict
            interaction_dict = self.history_log['trajectory'][f"Interaction Turn {self.steps}"]

            if "Verify EarlyExit" not in interaction_dict['Thought'] or interaction_dict.get("Action", "EXIT") != "EXIT":

                success = True
                action_dict = {
                    "response": f"Thought: {interaction_dict['Thought']}\nAction: {interaction_dict['Action']}",
                    "thought": interaction_dict['Thought'], # collect obs
                    "action": interaction_dict['Action'],
                    "token": interaction_dict['Token']
                }

                self.memory.append(
                    ("Action", f"Thought: {interaction_dict['Thought']}\nAction: {interaction_dict['Action']}")
                )
    ...
    def from_config(cls, llm_model, config):
        ...
        reflection_path = config.get("reflection_path", None)

        return cls(llm_model, memory_size, examples, instruction, init_prompt_path, system_message, 
                   need_goal, check_actions, check_inventory, use_parser, intrinsic_exit, history_file, reflection_path) 
