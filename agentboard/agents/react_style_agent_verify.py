...
@registry.register_agent("ReactStyleAgentVerify") # Agent with react format
class ReactStyleAgentVerify(BaseAgent):  # the agent should receive goal, state and action, then return the next state
    def __init__(self,
                 llm_model,
                 memory_size=100,
    ...
    def run(self, init_prompt_dict=None):
        ...
        if self.extrinsic_exit is not None and self.extrinsic_tool.exit_flag is True:
            action_dict = {
                "response": "extrinsic_exit", # record checklist in response
                "thought": "Verify EarlyExit.",
                "action": "EXIT",
                "token": self.extrinsic_tool.token_cnt
            }
