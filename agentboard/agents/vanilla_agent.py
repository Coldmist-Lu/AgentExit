...
class VanillaAgent(
    ...
    def __init__(self,
        ...
        if "claude" in self.llm_model.engine:
            self.split = self.llm_model.xml_split # self.split seems to be only used in claude engine
        else:
            self.split = {"example": [""],
                          "text": [""],
                          "rule": [""],
    ...
    def make_prompt(self, need_goal=False, check_actions="check valid actions", check_inventory="inventory", system_message=''):
        ...
        # load history -> from query to input prompt
        ...
        while num_of_tokens > self.max_context_length - self.llm_model.max_tokens:
            # adjust history length to make sure context within max length
            history = history[1:]
            input_prompt = query + "\n".join([item[0] + ": " + item[1] for item in history])
            input_prompt += "\nAction: "
    ...
    def action_parser_for_special_llms(self, action):
        ...
        if 'action' in action.lower():
            ...
            for act in action_temp: # reverse the action select
                if "next action" in act and ':' in act and (act.strip().endswith(':') is False): # zzh: in Claude will return "Here is the next action to take:" # The action should not end with ':'
    ...
    def run(self, init_prompt_dict=None):
        ...
        for _iter in range(3): # retry 3 times

            success, action = self.llm_model.generate(system_message, input_prompt)

            if success and self.use_parser: # if parse unsuccessful -> retry generation
                try:
                    action = self.action_parser_for_special_llms(action)

                except Exception as e: # capture exception
                    print(f"[ERROR] Failed to parse action. Exception: {e}")
                    print(f"[DEBUG] Original action input: {action}")
                    continue
